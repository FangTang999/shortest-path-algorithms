...
# FW Algorithm
def CalculateAPSPFloyd(G, numNode):
    fw_dist = list(map(lambda p: list(map(lambda q: q, p)), G))

    # Adding vertices individually. O(N^3)
    for r in range(numNode):
        for p in range(numNode):
            for q in range(numNode):
                fw_dist[p][q] = min(fw_dist[p][q], fw_dist[p][r] + fw_dist[r][q])
    # sol(fw_dist, numNode)

# Printing the output of FW
def sol(fw_dist,numNode):
    for p in range(numNode):
        for q in range(numNode):
            if(fw_dist[p][q] == MAX_LABEL):
                print("INF", end=" ")
            else:
                print(fw_dist[p][q], end="  ")
        print(" ")
...
def CalculateAPSP(method='dij'):
    ...
    elif method.lower().startswith('dij_heap'):
        for i in range(numNode):
    ...
    elif method.lower().startswith('deq_bilist'):
        for i in range(numNode):
    ...
    elif method.lower().startswith('fifo_ind'):
        for i in range(numNode):
            CalculateSSSPFIFOII(i, numNode, dist_apsp[i], pred_apsp[i])
    elif method.lower().startswith('fw'):
        print("Implemented by Alicia Tang --Aug/26/2021")
        CalculateAPSPFloyd(GetG(numNode), numNode)
